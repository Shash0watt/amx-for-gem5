...
main()
{
    // Do math, if zeroing dosen't work the dot product would produce 24.0.
    ...
    // TMM0 contains 1.0. TILEZERO must complete before this
    // dependent accumulation may issue, so the trace after the dot product
    // must show 23.0 = (2.0 * 4.0) + (3.0 * 5.0), not 24.0.
    amx_tile_zero(0);
